import { createUser } from "@/src/api/auth/service";
...
const Register = memo(() => {
    ...
    const onSubmit = useCallback(async () => {
        ...
        try {
            let msg = '';
            if (emailVal.current === '' || usernameVal.current === '' || passwordVal.current === '' || rePasswordVal.current === '')
            ...
            else if (usernameVal.current.length > 10)
                msg = 'username max length 10';
    ...
    return (
        <View style={[gs.full_size]}>
            <View style={[gs.f1, gs.full_size]}>
                <GeneralHeaderBarComponent title="REGISTER" />
            </View>
            <View style={[gs.f9, gs.full_size]}>
                ...
                <View style={[gs.f1, gs.p5]}>
                    <View style={[gs.border_card, gs.full_size, gs.p5]}>
                        <Text style={[gs.f1, styles.label, gs.fontM]}>USERNAME</Text>
                        <TextInput style={[gs.f2, gs.fontM]}
                            placeholder="Enter username"
                            onChangeText={(text) => { usernameVal.current = text; }}
                            returnKeyType="next"
                        />
                    </View>
                </View>
