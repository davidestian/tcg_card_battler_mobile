...
export const invGetPlayerUnits = async (limit: number, page: number): Promise<APIResponse<InvGetPlayerUnitRS>> => {
...
export const invGetAllPlayerCard = async (limit: number, cursorPrice: number, cursorCardCode: string, cursorImageTypeCode: number, isPrev: boolean, pageNumber: number): Promise<APIResponse<InvGetAllPlayerCardRS>> => {
    const response = await privateAPI.get<APIResponse<InvGetAllPlayerCardRS>>(`${PathURL}/card?limit=${limit}&price=${cursorPrice}&code=${cursorCardCode}&imageTypeNumber=${cursorImageTypeCode}&isPrev=${isPrev}&pageNumber=${pageNumber}`);
    return response.data;
}
